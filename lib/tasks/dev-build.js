...
gulp.task('generate_modules', ['modules:check_non_module_folders'], done => {
    ...
    let appModuleCode = buildAppModuleCode(config.modules)
        .concat('')
        .join(os.EOL);
    fs.writeFileSync(`${config.folders.modules}appx.ts`, appModuleCode);
...
function buildAppModuleCode(modules) {
    ...
    let envConfig = require(config.config.src);
    console.log(JSON.stringify(envConfig, null, 4));
    iterateProperties(envConfig, code, 8);
...
gulp.task('compile_scripts', ['generate_modules', tsks.definitions.generate], () => {
    ...
    compileTask = compileTask.pipe($.typescript(config.options.typescriptBuild));
